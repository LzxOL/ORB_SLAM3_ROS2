...
void RgbdSlamNode::ProcessFrame()
{
    while (running_) {
        ...
        auto now = this->now();
        if ((now - last_pointcloud_time_).seconds() >= pointcloud_publish_interval_) {
            auto depthValueScale = depth_frame->getValueScale();
            pointCloud.setPositionDataScaled(depthValueScale);
            pointCloud.setCreatePointFormat(OB_FORMAT_RGB_POINT);
            std::shared_ptr<ob::Frame> frame = pointCloud.process(frameset); // 生成点云
            if (pointcloud_publisher_ && pointcloud_publisher_->get_subscription_count() > 0) {
                if (frame) {
                    auto cloud_msg = convertToRosPointCloud(frame);
...
sensor_msgs::msg::PointCloud2 RgbdSlamNode::convertToRosPointCloud(std::shared_ptr<ob::Frame> frame) {
    ...
    uint64_t sensor_time_us = points_frame->systemTimeStampUs();
    cloud.header.stamp = rclcpp::Time(sensor_time_us);
    cloud.header.frame_id = pointcloud_frame_id_;
    ...
    const float min_distance = 0.02f; // 20mm (0.02m)
    const float max_distance = 10.0f;  // 10m
    size_t valid_count = 0;
...
//     // 创建ROS点云消息
//     sensor_msgs::msg::PointCloud2 cloud;
//     cloud.header.stamp = this->now();  // 使用当前时间而不是传感器时间
//     cloud.header.frame_id = pointcloud_frame_id_;
    
//     // 设置点云字段
//     cloud.fields.resize(4);
//     cloud.fields[0].name = "x"; cloud.fields[0].offset = 0; 
//     cloud.fields[0].datatype = sensor_msgs::msg::PointField::FLOAT32; cloud.fields[0].count = 1;
//     cloud.fields[1].name = "y"; cloud.fields[1].offset = 4;
//     cloud.fields[1].datatype = sensor_msgs::msg::PointField::FLOAT32; cloud.fields[1].count = 1;
//     cloud.fields[2].name = "z"; cloud.fields[2].offset = 8;
//     cloud.fields[2].datatype = sensor_msgs::msg::PointField::FLOAT32; cloud.fields[2].count = 1;
//     cloud.fields[3].name = "rgb"; cloud.fields[3].offset = 12;
//     cloud.fields[3].datatype = sensor_msgs::msg::PointField::UINT32; cloud.fields[3].count = 1;
    
//     // 设置点云结构
//     cloud.point_step = 16; // 4 bytes * 4 fields
//     cloud.row_step = cloud.point_step * total_points;
...
//     // 获取点云数据指针
//     auto* cloud_data = reinterpret_cast<float*>(cloud.data.data());
    
//     // 并行处理点云数据
//     #pragma omp parallel for
//     for (size_t i = 0; i < total_points; ++i) {
//         const auto& p = points[i];
//         float* point_ptr = cloud_data + i * 4;  // 每个点4个float值
        
//         // 毫米转米
//         point_ptr[0] = p.x * 0.001f;  // x
//         point_ptr[1] = p.y * 0.001f;  // y
//         point_ptr[2] = p.z * 0.001f;  // z
...
//         // 使用memcpy确保正确的字节序
//         memcpy(&point_ptr[3], &rgb, sizeof(uint32_t));
//     }
